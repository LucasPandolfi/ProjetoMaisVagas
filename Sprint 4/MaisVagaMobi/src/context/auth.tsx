...
export const AuthProvider: React.FC = ({ children }: any) => {
    ...
    const checkUserLogged = (response: any) => {
        ...
        if (!response) {
            ...
            tokenDecoded?.Role === "Candidato" ? (setIsCandidato(true), setIsEmpresa(false)) : (setIsCandidato(false), setIsEmpresa(true));
    ...
    const SignIn = async (form: LoginProps) => {
        ...
        await fetch('http://localhost:5000/api/login', init)
            .then(resp => resp.json())
            .then(data => {

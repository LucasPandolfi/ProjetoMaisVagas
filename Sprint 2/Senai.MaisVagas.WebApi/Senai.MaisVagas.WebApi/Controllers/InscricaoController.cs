...
namespace Senai.MaisVagas.WebApi.Controllers
{
    ...
    public class InscricaoController : ControllerBase
    {
        ...
        [HttpGet("inscricaoCandidato/{id}")]
        public IActionResult ListarInscricoesPorCandidato(int id)
        {
            try
            {
                return Ok(_inscricaoRepository.ListarInscricoesPorCandidato(id));
            }
            catch (Exception error)
            {
                return BadRequest(error);
        ...
        public IActionResult Candidatura(Inscricao novaInscricao)
        {
            try
            {
                //passo 1 verificar se existe um candidato com esse idUsuario

                var inscricao = _inscricaoRepository.Candidatura(novaInscricao);
                if (inscricao == null)
                {
                    return StatusCode(400, "O candidato não existe");
                }
